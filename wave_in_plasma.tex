...
\begin{itemize}
	...
	\item $\omega=\omega_{pe}$となるプラズマ密度$n_e = \frac{m\omega^2_{pe}}{4\pi e^2}$（臨界密度）で電磁波が遮断される。プラズマは$\omega > \omega_{pe}$を満たす高周波の波しか通さない。{\color{red}物理的にはなぜ？}
	      ...
	      \end{itemize}
\end{itemize}
\subsection{正常波：電場と背景磁場が平行な場合の電子波}
...
\subsection{異常波：電場と背景磁場が垂直な場合の電子波}
...
\subsection{磁気音波：背景磁場と垂直な方向に進行する低周波電磁波（修正されたAlfven波）}
...
\begin{equation}
	...
	= \frac{ie}{M\omega}E_{ix}\qty(1-\frac{\Omega^2_c}{\omega^2}\frac{1}{1-A_i})^{-1}
\end{equation}
を得る。また、$\omega \ll \Omega_c$を仮定すると、
\begin{equation}
	v_{ix} \approx -\frac{ie}{M\omega}E_{ix}\qty(\frac{\Omega^2_c}{\omega^2}\frac{1}{1-A_i})^{-1}
	= -\frac{ie}{M\omega}E_{ix}\frac{\omega^2}{\Omega_c^2}\qty(1-\frac{k^2}{\omega^2}\frac{\gamma_ik_{\text{B}}T_i}{M})
\end{equation}
と計算できる。電子についても同様の運動方程式が成立する。また、電子の熱速度が波の位相速度より十分に早いと仮定する：
\begin{equation}
	\frac{\omega^2}{k^2} \ll v_{\text{th}}^2 \approx k_{\text{B}}T_i\;。
\end{equation}
すなわち、
\begin{equation}
	v_{ex} \approx \frac{ie}{m\omega}E_{ix}\qty(\frac{\Omega^2_c}{\omega^2}\frac{1}{1-A_e})^{-1}
	= \frac{ie}{m\omega}E_{ix}\frac{\omega^2}{\Omega_c^2}\qty(1-\frac{k^2}{\omega^2}\frac{\gamma_ik_{\text{B}}T_i}{M})
\end{equation}

\begin{equation}
	\approx \frac{ie}{M\omega}E_x\frac{\omega^2M^2}{e^2B_0^2}\frac{k^2}{\omega^2}\frac{\gamma_ik_{\text{B}}T_i}{M} = \frac{ik^2}{\omega{}B_0^2}\frac{\gamma_ik_{\text{B}}T_i}{e}E_x
\end{equation}




\subsection{外部磁場と波数ベクトルが平行な場合の電子波}
...
\subsection{Alfven波：背景磁場と平行な方向に進行する低周波電磁波（シアーAlfven波）}
...
\end{align}
この極限では電子のLarmor回転は無視することができ、$y$方向に$\bm{E}\cross\bm{B}$ドリフトすることになる。$\bm{v}_{ix},\,\bm{v}_{ex}(=0)$をMaxwell方程式に代入して、
\begin{equation}
